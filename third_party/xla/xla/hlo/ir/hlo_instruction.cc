...
absl::Status HloInstruction::ReplaceAllUsesWith(HloInstruction* new_producer,
                                                HloInstruction* except) {
  TF_RET_CHECK(
      ShapeUtil::CompatibleIgnoringFpPrecision(shape(), new_producer->shape()))
      << shape() << " is not compatible with " << new_producer->shape();
  return ReplaceAllUsesWithDifferentShape(new_producer, except);
}

absl::Status HloInstruction::ReplaceAllUsesWithDifferentShape(
    HloInstruction* new_producer, HloInstruction* except) {
  std::vector<HloInstruction*> keep;
  ...
  // instruction. However, we do need to keep the new_producer and the except in
  // the users list of this instruction.
  if (users_.Contains(new_producer)) keep.push_back(new_producer);
  if (except != nullptr) keep.push_back(except);
  for (auto* user : keep) RemoveUser(user);
  ...
  // If there were other uses of new_producer, they now in the users_ list.
  // We need to move them back to the new_producer.users list.
  for (HloInstruction* user : users_.vec()) new_producer->AddUser(user);
  users_.Clear();

  // Now re-add the users that we want to keep.
  for (HloInstruction* user : keep) AddUser(user);
